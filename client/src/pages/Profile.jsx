...
const Profile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/users/update/${currentUser._id}`, {
        method: 'POST',
        headers: {
  ...
  const handleShowListings = async () => {
    try {
      ...
      const res = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/users/listings/${currentUser._id}`,{
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleListingDelete = async (listingId) => {
    try {
      const res = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/listing/delete/${listingId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <input type="password" placeholder='password' defaultValue="" className='border p-3 rounded-lg' onChange={handleChange} id="password"/>
        <button
          disabled={loading}
